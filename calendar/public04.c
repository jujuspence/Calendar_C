...
int main(void) {
   ...
   add_event(calendar, "zoo visit", start_time_mil, duration_minutes,              info, activity_day);
   ...
   if (find_event(calendar, "review meeting", &event) == SUCCESS) {      printf("Event found: %s, %d, %d\n", event->name, event->start_time,             event->duration_minutes);
   } else {
      printf("event \"review meeting\" not found\n");
   }
   
   if (find_event(calendar, "weekend meeting", &event) == SUCCESS) {      printf("%s, %d, %d\n", event->name, event->start_time, event->duration_minutes);   } else {
